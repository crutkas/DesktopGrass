...
- Generated by `set_scene` for hard scene entities and by per-scene emitters for soft weather (Grass raindrops, Winter snowflakes).
...
emit new snowflakes per §15 schedule and new raindrops per §20 schedule
```
