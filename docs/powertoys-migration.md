...
| Managed implementation | Keep the C#/Vortice source and build/unit coverage only as a reproducible comparison/reference. Do not import it or its graphics dependencies. |
...
for Native standalone releases or PowerToys readiness.
...
| PT-004 | Ready | Set resource budgets and measurement method | PT-003 | Maintainers approve CPU, GPU, working-set, wakeup, and energy-impact limits; test duration, scenes, hardware, and pass/fail calculations are recorded. |
