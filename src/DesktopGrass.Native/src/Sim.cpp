...
void sim_apply_click(Sim& sim, const InputEvent& e) noexcept {
    ...
    for (Entity& ent : sim.entities) {
        if (ent.kind != EntityKind::Raccoon) continue;
        const double dxClick = ent.x - e.x;
        const double dyClick = ent.y - e.y;
        if ((dxClick * dxClick + dyClick * dyClick) > (RACCOON_STARTLE_RADIUS * RACCOON_STARTLE_RADIUS)) continue;
        const double awayDir = dxClick >= 0.0 ? 1.0 : -1.0;
        const double baseSpeed = std::clamp(ent.rotationSpeed, RACCOON_WALK_SPEED_MIN, RACCOON_WALK_SPEED_MAX);
        ent.vx = awayDir * baseSpeed * RACCOON_STARTLE_BOOST;
        ent.state = RACCOON_STATE_WALKING;
        ent.stateTimer = RACCOON_SNUFFLE_DURATION_MAX;
        ent.age = 0.0;
    }
}
...
void generate_critters_raccoon(Sim& sim, bool allowOverride) noexcept {
    const int count = resolve_critter_count(sim, RACCOON_COUNT_MIN, RACCOON_COUNT_MAX, allowOverride);
    const double groundY = sim.windowHeight;
    for (int i = 0; i < count
         && static_cast<int>(sim.entities.size()) < MAX_ENTITIES_PER_MONITOR; ++i) {
        Entity e{};
        e.kind = EntityKind::Raccoon;
        e.size = RACCOON_BODY_RADIUS;
        const double margin = e.size + RACCOON_TAIL_LENGTH + 4.0;
        const double usableWidth = std::max(0.0, sim.monitorWidth - 2.0 * margin);
        e.x = margin + prng_uniform(sim.critterPrng, 0.0, 1.0) * usableWidth;
        const double speed = prng_uniform(sim.critterPrng, RACCOON_WALK_SPEED_MIN, RACCOON_WALK_SPEED_MAX);
        const double dir = (prng_next_u64(sim.critterPrng) & 1ull) != 0ull ? 1.0 : -1.0;
        e.vx = dir * speed;
        e.rotationSpeed = speed;
        e.lifetime = -1.0;
        e.seed = static_cast<uint32_t>(i + 1);
        e.state = RACCOON_STATE_WALKING;
        e.stateTimer = prng_uniform(sim.critterPrng, RACCOON_WALK_DURATION_MIN, RACCOON_WALK_DURATION_MAX);
        e.nameIndex = i == 0
            ? 0
            : static_cast<uint8_t>(1 + prng_index(sim.critterPrng,
                static_cast<uint32_t>(sizeof(RACCOON_NAME_POOL) / sizeof(RACCOON_NAME_POOL[0]) - 1)));
        e.y = groundY - RACCOON_BODY_HEIGHT - RACCOON_LEG_LENGTH;
        sim.entities.push_back(e);
    }
}
...
void generate_grass_critters_all(Sim& sim) noexcept {
    ...
    generate_critters_raccoon(sim, false);
}
...
void generate_critters_for_kind(Sim& sim) noexcept {
    ...
    case CritterKind::Raccoon: generate_critters_raccoon(sim, true); break;
    }
...
void sim_tick_entities(Sim& sim, double dt) noexcept {
    ...
    // Raccoons alternate between waddling, snuffling, and resting.
    for (Entity& e : sim.entities) {
        ...
        const double margin = e.size + RACCOON_TAIL_LENGTH + 2.0;
        if (e.x < margin) {
            e.x = margin;
            e.vx = std::abs(e.vx);
        } else if (e.x > sim.monitorWidth - margin) {
        ...
        if (e.stateTimer <= 0.0) {
            if (e.state == RACCOON_STATE_WALKING) {
                if (prng_uniform(sim.critterPrng, 0.0, 1.0) < RACCOON_SNUFFLE_PROBABILITY) {
                    e.state = RACCOON_STATE_SNUFFLING;
                    e.stateTimer = prng_uniform(sim.critterPrng,
                        RACCOON_SNUFFLE_DURATION_MIN, RACCOON_SNUFFLE_DURATION_MAX);
                } else {
                    e.state = RACCOON_STATE_RESTING;
                    e.stateTimer = prng_uniform(sim.critterPrng,
                        RACCOON_REST_DURATION_MIN, RACCOON_REST_DURATION_MAX);
                }
            } else {
                e.state = RACCOON_STATE_WALKING;
                e.stateTimer = prng_uniform(sim.critterPrng,
                    RACCOON_WALK_DURATION_MIN, RACCOON_WALK_DURATION_MAX);
            }
            e.age = 0.0;
        }
    }
