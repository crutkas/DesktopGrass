...
{
TEST_CLASS(WinterTests)
{
public:
TEST_METHOD(WinterConstantsArePinned) {
    Assert::IsTrue(SNOWFLAKE_EMIT_RATE_PER_1920DIP == Near(8.0));
    Assert::IsTrue(SNOWFLAKE_FALL_SPEED_MIN == Near(20.0));
    Assert::IsTrue(SNOWFLAKE_FALL_SPEED_MAX == Near(40.0));
    Assert::IsTrue(SNOWFLAKE_SIZE_MIN == Near(1.5));
    Assert::IsTrue(SNOWFLAKE_SWAY_AMPLITUDE == Near(10.0));
    Assert::IsTrue(SNOWFLAKE_PRNG_SALT == 0xC0FFEE1CECAFEBABull);
    Assert::IsTrue(SNOW_TIP_RADIUS_FACTOR == Near(1.25));
    Assert::IsTrue(SNOW_TIP_COLOR == 0xFFFFFFFFu);
}

TEST_METHOD(WinterBladeCullIsDeterministicAnd25) {
    ...
    for (uint32_t i = 0; i < 32; ++i) {
        ...
        Assert::IsTrue(winter_blade_culled(i) == expected);
    }

    Assert::IsTrue(WINTER_CULL_MASK == 3u);

    int culled = 0;
    for (uint32_t i = 0; i < 2500; ++i) {
        if (winter_blade_culled(i)) ++culled;
    }
    Assert::IsTrue(culled == 624); // 24.96% of 2500 — effectively the target 25%
}

TEST_METHOD(SetSceneWinterInitializesSnowflakeScheduler) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);

    Assert::IsTrue(sim.nextSnowflakeSpawnTime > sim.globalTime);
    Assert::IsTrue(sim.nextSnowflakeSpawnTime < 100.0);
}

TEST_METHOD(FirstWinterSnowflakeEmitsOnScheduledTick) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    Assert::IsTrue(sim.entities[0].kind == EntityKind::Snowflake);
}

TEST_METHOD(FirstWinterSnowflakeMatchesSpecDerivedPRNGSnapshot) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    Assert::IsTrue(e.vy == Near(expectedFallSpeed).margin(1e-12));
    Assert::IsTrue(e.rotation == Near(expectedRotation).margin(1e-12));
    Assert::IsTrue(e.rotationSpeed == Near(expectedRotationSpeed).margin(1e-12));
    Assert::IsTrue(e.seed == expectedSeed);
    Assert::IsTrue(sim.nextSnowflakeSpawnTime == Near(firstInterval + nextInterval).margin(1e-12));
}

TEST_METHOD(SnowflakeSwayVelocityWobblesFromSeedPhase) {
    Sim sim = MakeWinterTestSim();
    ...
    Assert::IsTrue(sim.entities.size() == 1);
    Assert::IsTrue(sim.entities[0].vx == Near(expectedVx).margin(1e-12));
}

TEST_METHOD(SnowflakesAreCulledAfterLifetime) {
    Sim sim = MakeWinterTestSim();
    sim.currentScene = Scene::Desert;
    Entity e{};
    e.kind = EntityKind::Snowflake;
...
TEST_METHOD(SnowflakesAreCulledBelowGroundLine) {
    Sim sim = MakeWinterTestSim();
    sim.currentScene = Scene::Desert;
    Entity e{};
    e.kind = EntityKind::Snowflake;
...
TEST_METHOD(WinterSnowflakeEmitterHonorsMaxEntityCap) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    Assert::IsTrue(sim.entities.size() <= static_cast<std::size_t>(MAX_ENTITIES_PER_MONITOR));
    Assert::IsTrue(sim.entities.size() == static_cast<std::size_t>(MAX_ENTITIES_PER_MONITOR));
}

TEST_METHOD(WinterSceneDoesNotPerturbFirstBladeSnapshot) {
    Sim sim = MakeWinterTestSim();
    Assert::IsTrue(sim.blades[0].baseX == Near(CANONICAL_FIRST_10[0].baseX).margin(1e-12));
...
TEST_METHOD(SnowflakesDoNotEmitInNonWinterScenes) {
    Sim sim = MakeWinterTestSim();

    sim_set_scene(sim, Scene::Grass);
    sim.nextSnowflakeSpawnTime = 0.0;
    sim_tick(sim, 2.0, nullptr, 0);
    Assert::IsTrue(std::none_of(sim.entities.begin(), sim.entities.end(),
...
namespace DesktopGrassNativeTests
{
TEST_CLASS(WinterPuffTests)
{
public:
TEST_METHOD(SnowPuffConstantsArePinned) {
    Assert::IsTrue(SNOW_PUFF_COUNT_MIN == 9);
    Assert::IsTrue(SNOW_PUFF_COUNT_MAX == 16);
    Assert::IsTrue(SNOW_PUFF_SIZE_MIN == Near(3.5));
    Assert::IsTrue(SNOW_PUFF_SIZE_MAX == Near(8.0));
    Assert::IsTrue(SNOW_PUFF_GRAVITY == Near(150.0));
    Assert::IsTrue(SNOW_PUFF_DRAG == Near(1.6));
    Assert::IsTrue(SNOW_PUFF_SPREAD_RAD == Near(1.25));
    Assert::IsTrue(SNOW_PUFF_PRNG_SALT == 0x5503FF1E5503FF1Eull);
}

TEST_METHOD(ClickingTheWinterSnowbankShedsASnowPuffBurst) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    Assert::IsTrue(puffs >= SNOW_PUFF_COUNT_MIN);
    Assert::IsTrue(puffs <= SNOW_PUFF_COUNT_MAX);
    ...
    for (const Entity& e : sim.entities) {
        ...
        Assert::IsTrue(e.vy < 0.0);
        Assert::IsTrue(e.y <= sim.windowHeight + 1e-9);
    }
}

TEST_METHOD(SnowPuffOnlyFiresInWinter) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Grass);
    ...
    Assert::IsTrue(count_snow_puffs(sim) == 0);
}

TEST_METHOD(ANonFiniteClickShedsNoSnowPuff) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
...
TEST_METHOD(SnowPuffBurstRisesThenSettlesAndIsCulled) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
...
TEST_METHOD(SnowPuffDrawOrderMatchesASidePRNGStream) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    Assert::IsTrue(count_snow_puffs(sim) == expectedCount);
...
TEST_METHOD(SnowPuffSaltIsUniqueAmongWinterPRNGSalts) {
    ...
    for (uint64_t s : otherSalts) {
        Assert::IsTrue(SNOW_PUFF_PRNG_SALT != s);
    }
}
...
namespace DesktopGrassNativeTests
{
TEST_CLASS(WinterDriftTests)
{
public:
TEST_METHOD(SnowDriftConstantsArePinned) {
    Assert::IsTrue(SNOW_DRIFT_COUNT_MIN == 4);
    Assert::IsTrue(SNOW_DRIFT_COUNT_MAX == 8);
    Assert::IsTrue(SNOW_DRIFT_REACH_DIP == Near(70.0));
    Assert::IsTrue(SNOW_DRIFT_MIN_SPEED == Near(90.0));
    Assert::IsTrue(SNOW_DRIFT_COOLDOWN_SEC == Near(0.12));
    Assert::IsTrue(SNOW_DRIFT_SIZE_SCALE == Near(0.9));
    Assert::IsTrue(SNOW_DRIFT_SPEED_SCALE == Near(0.85));
    Assert::IsTrue(SNOW_DRIFT_PRNG_SALT == 0x5D81F77D5D81F77Dull);
}

TEST_METHOD(BrushingTheCursorAcrossTheSnowbankKicksUpADriftWisp) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
    ...
    const int puffs = count_snow_puffs(sim);
    Assert::IsTrue(puffs >= SNOW_DRIFT_COUNT_MIN);
    Assert::IsTrue(puffs <= SNOW_DRIFT_COUNT_MAX);

    // Drift grains are smaller than a click burst and still launch upward.
    for (const Entity& e : sim.entities) {
        if (e.kind != EntityKind::SnowPuff) continue;
        Assert::IsTrue(e.vy < 0.0);
        Assert::IsTrue(e.size <= SNOW_PUFF_SIZE_MAX * SNOW_DRIFT_SIZE_SCALE + 1e-9);
        ...
        Assert::IsTrue(e.y <= groundY + 1e-9);
        Assert::IsTrue(e.y >= groundY - SNOW_PUFF_START_RADIUS - 1e-9);
    }
}

TEST_METHOD(SnowDriftOnlyFiresInWinter) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Grass);
...
TEST_METHOD(ASlowCursorBrushKicksUpNoDrift) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
...
TEST_METHOD(AHighCursorBrushAboveTheSnowBandKicksUpNoDrift) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);
...
TEST_METHOD(SnowDriftRespectsTheGlobalCooldown) {
    Sim sim = MakeWinterTestSim();
    sim_set_scene(sim, Scene::Winter);

    WinterDrift(sim, 300.0, 360.0, 0.05);
    const int first = count_snow_puffs(sim);
    Assert::IsTrue(first >= SNOW_DRIFT_COUNT_MIN);
    ...
    Assert::IsTrue(count_snow_puffs(sim) == first);
...
TEST_METHOD(SnowDriftMovesLeaveTheClickPuffStreamUntouched) {
    ...
    Assert::IsTrue(aClick.size() == bClick.size());
    for (std::size_t i = 0; i < aClick.size(); ++i) {
        Assert::IsTrue(aClick[i].size == Near(bClick[i].size).margin(1e-12));
        Assert::IsTrue(aClick[i].vx == Near(bClick[i].vx).margin(1e-12));
        Assert::IsTrue(aClick[i].vy == Near(bClick[i].vy).margin(1e-12));
        Assert::IsTrue(aClick[i].lifetime == Near(bClick[i].lifetime).margin(1e-12));
    }
}
};
}
