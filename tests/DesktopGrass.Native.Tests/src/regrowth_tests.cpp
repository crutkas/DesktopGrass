...
{
TEST_CLASS(RegrowthTests)
{
public:
TEST_METHOD(CutCompletionSchedulesRegrowth) {
    ...
    Assert::IsTrue(sim.blades[0].regrowStart  == Near(1.2).margin(1e-9));
}

TEST_METHOD(RegrowthIsLinearOverRegrowDuration) {
    ...
    for (int i = 0; i < 4; ++i) sim_tick(sim, 0.05, nullptr, 0);
    Assert::IsTrue(sim.blades[0].regrowStart == Near(0.70).margin(1e-9));
    ...
    sim_tick(sim, 0.10, nullptr, 0);
    Assert::IsTrue(sim.blades[0].cutHeight  == Near(1.0).margin(1e-9));
    Assert::IsTrue(sim.blades[0].regrowStart  < 0.0);
...
TEST_METHOD(ReClickDuringRegrowthRestartsTheCutFromCurrentHeight) {
    ...
    // regrowStart cleared.
    Assert::IsTrue(sim.blades[0].cutAnimStart    >= 0.0);
    Assert::IsTrue(sim.blades[0].cutInitialHeight == Near(0.5).margin(1e-9));
    Assert::IsTrue(sim.blades[0].regrowStart      < 0.0);
...
TEST_METHOD(ClickOnStumpCutWaitingToRegrowIsANoOp) {
    ...
    Assert::IsTrue(sim.blades[0].cutHeight    == Near(0.0));
    Assert::IsTrue(sim.blades[0].cutAnimStart  < 0.0);
    Assert::IsTrue(sim.blades[0].regrowStart  == Near(5.0));
}

TEST_METHOD(RegrowthJitterIsDeterministicForAGivenSeed) {
    std::vector<Blade> a;
    std::vector<Blade> b;
    ...
    for (std::size_t i = 0; i < a.size(); ++i) {
        Assert::IsTrue(a[i].regrowDelay    == Near(b[i].regrowDelay   ).margin(1e-12));
        Assert::IsTrue(a[i].regrowDuration == Near(b[i].regrowDuration).margin(1e-12));
    }
}

TEST_METHOD(RegrowthJitterFallsWithinConfiguredMinMax) {
    std::vector<Blade> blades;
    generate_blades(CANONICAL_TEST_SEED, 1920.0, 1.0, blades);

    Assert::IsTrue(blades.size() > 50);
    for (const Blade& b : blades) {
        Assert::IsTrue(b.regrowDelay    >= REGROW_DELAY_MIN);
        Assert::IsTrue(b.regrowDelay    <  REGROW_DELAY_MAX);
        Assert::IsTrue(b.regrowDuration >= REGROW_DURATION_MIN);
        Assert::IsTrue(b.regrowDuration <  REGROW_DURATION_MAX);
        Assert::IsTrue(b.regrowStart    == Near(-1.0));
    }
}

TEST_METHOD(RegrowthJitterDoesNotPerturbStaticFieldGeneration) {
    ...
    Assert::IsTrue(a[0].baseX    == Near(a[0].baseX));   // tautology — placeholder
}
};
}
